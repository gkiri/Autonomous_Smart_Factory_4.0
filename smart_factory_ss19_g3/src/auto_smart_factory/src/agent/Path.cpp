...
Path::Path(std::string agent_id, int task_id, GOAL path_goal, geometry_msgs::Point start_position,
           geometry_msgs::Point drive_point,
           geometry_msgs::Point approach_point, geometry_msgs::Point direction_point,
           geometry_msgs::Point drive_back_point, bool idle) {

    agentID = agent_id;
    taskID = task_id;
    startPosition = start_position;
    drivePoint = drive_point;
    this->idle = idle;
    pathGoal = path_goal;
    done = false;
    firstChunk = true;
...
geometry_msgs::Point Path::getDrivePoint() {
    return drivePoint;
}
