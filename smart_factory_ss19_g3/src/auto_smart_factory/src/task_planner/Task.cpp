...
Task::Task(unsigned int id, TaskData taskData) :
	...
	ROS_INFO("[Task] New task with ID %d: %s carries package %d from tray %d to tray %d, duration: %f", getId(), state.robot.c_str(), state.package.id, state.sourceTray, state.targetTray, state.estimatedDuration.toSec());
}
