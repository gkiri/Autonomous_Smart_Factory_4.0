...
def create_id_texts(tray, tray_id, x, y, z, orientation, tray_counts):
    ...
    if tray['type'] == 'input':
        txt_id = tray_id - (tray_counts[0] - 1) # charging station sum - 1
    if tray['type'] == 'output':
        txt_id = tray_id - (tray_counts[0] + tray_counts[1] + tray_counts[2] - 1) # all trays except output
    if tray['type'] == 'storage':
        txt_id = tray_id - (tray_counts[0] + tray_counts[1] - 1)
    if tray['type'] == 'charging station':
        txt_id = tray_id + 1

    if txt_id <= 20:  # only numbers upto 20 exists as blender text drawings.
        tray_id_txt = str(txt_id)
...
def create_tray(tray_id, tray, simply_flag, tray_counts):
    # determine model
    if tray['type'] == 'input':
    ...
    # add ids of trays
    # create_id_texts(tray, tray_id, tray['x'], tray['y'], 0, tray['orientation'], tray_counts)
...
def create_charging_station(tray_id, tray, tray_counts):
...
def setup_warehouse_map(map_config, robot_config, simplfy_flag):
    ...
    for tray_id, tray in enumerate(map_config['trays']):
        if tray['type'] == 'charging station':
            tray_counts[0] += 1
        elif tray['type'] == 'input':
            tray_counts[1] += 1
        elif tray['type'] == 'storage':
            tray_counts[2] += 1
        elif tray['type'] == 'output':
            tray_counts[3] += 1

    for tray_id, tray in enumerate(map_config['trays']):
        if tray['type'] == 'charging station':
            create_charging_station(tray_id, tray, tray_counts)
        else:
            create_tray(tray_id, tray, simplfy_flag, tray_counts)
