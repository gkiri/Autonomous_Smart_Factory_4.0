...
from auto_warehousing.msg import Wall
...
class WarehouseConfigGenerator(object):
    ...
    def generateWall(self, x, y, orientation):
        w = copy.deepcopy(WarehouseConfigGenerator.wallPrototype)
    ...
    def generateWallRow(self, x, y, n, facingUp = True):
            for i in range(n):
                if facingUp:
                    orientation = 180
                else:
                    orientation = 0
                self.generateWall(x + i * WarehouseConfigGenerator.wallGeometry['width'], y, orientation)

    def generateWallColumn(self, x, y, n, facingLeft = True):
        for i in range(n):
            if facingLeft:
                orientation = 90
            else:
                orientation = -90
            self.generateWall(x, y + i * WarehouseConfigGenerator.wallGeometry['height'], orientation)
    ...
    def generateHBarw(self, x, y, n):
        self.generateWallRow(x, y, n, False)
        self.generateWallRow(x, y + WarehouseConfigGenerator.wallGeometry['height'], n, True)

    def generateVBarw(self, x, y, n):
        self.generateWallColumn(x, y, n, True)
        self.generateWallRow(x + WarehouseConfigGenerator.wallGeometry['width'], y, n, False)
    ...
    def saveConfig(self, file):
        config = {
            ...
            'wall_geometry' : {
                'width' : WarehouseConfigGenerator.wallGeometry['width'],
                'height' : WarehouseConfigGenerator.wallGeometry['height']
            },
            'trays' : self.trays,
            'walls' : self.walls,
            'robots' : self.robots
